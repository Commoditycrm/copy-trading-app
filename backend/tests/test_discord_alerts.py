...
# NOTE: the "copy-paused-at-trade-time" tests that used to live here were
# removed. They covered da._copy_paused_at, which reconstructed the trader's
# copy state from the audit trail so a fill that happened DURING a pause but
# surfaced after it would still be suppressed. That gate was deliberately
# dropped in b4937d1 ("alert every trade when ON, none when OFF") — alerting is
# now governed solely by discord_alerts_enabled, and ts.copy_paused only sets a
# display flag on the card. The tests outlived the function and had been failing
# ever since; there is nothing left for them to assert.
